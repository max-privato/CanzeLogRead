...
MainWindow::MainWindow(QWidget *parent) : QMainWindow(parent) ,
  ...
  if(QCoreApplication::arguments().count()>1)
     droppedFile=QCoreApplication::arguments().at(1);
...
void MainWindow::dropEvent(QDropEvent *event){
  ...
  processDroppedFile(droppedName);
}
...
void MainWindow::processDroppedFile(QString droppedName){
  /* Function for loading files because they are dropped with program open or closed
   * In the latter case the dropped file appears as passed parameter.
